...
energy_net.load_state_dict(torch.load("verkko3.pth"))
energy_net.eval()
...
x, y = 2.8, 8.9    # alkutila
positions = [(x, y)]
dt = 0.3

# Ulkoinen stimulaatio
x_target = 11
N = 200
kick_strength = 0.4
...
for i in range(300):
    dx, dy = gradient(x, y)
    if i < N:
        kick_force_x = (x_target - x) * kick_strength * decay[i]
    else:
        kick_force_x = 0

    x += dx * dt + kick_force_x * (dt / 2)
    y += dy * dt
