...
# Neuroverkko kahdelle attraktorille, ulkoinen stimulaatio
...
# Kaksi attraktoria, keskipisteet (3,9) ja (9,4)
background = 0.1 * np.cos(0.5 * x) + 0.1 * np.cos(0.5 * y)
attractor1 = -np.exp(-((x - 3)**2 + (y - 9)**2))
...
torch.save(energy_net.state_dict(), "verkko3.pth")