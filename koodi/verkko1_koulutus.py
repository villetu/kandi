...
# Neuroverkko yhdelle attraktorille
...
# Attraktori, keskipiste (5, 7)
background = 0.1 * np.cos(0.5 * x) + 0.1 * np.cos(0.5 * y)
attractor = -np.exp(-((x - 5)**2 + (y - 7)**2))
target_E = background + attractor
...
torch.save(energy_net.state_dict(), "verkko1.pth")